describe('MathUtilities Test', function()
{
    ...
    it('should truncate a time stamp to the exact hour; on exact time', function()
    {
        ...
        // THEN
        expect($filter('date')(timestamp, 'yyyy-MM-ddTHH:mm:ss', 'UTC')).toBe('2011-02-24T09:00:00');
        expect($filter('date')(rounded, 'yyyy-MM-ddTHH:mm:ss', 'UTC')).toBe('2011-02-24T09:00:00');
    });
    ...
    it('should truncate a time stamp to the exact hour; in the first half hour', function()
    {
        ...
        // THEN
        expect($filter('date')(timestamp, 'yyyy-MM-ddTHH:mm:ss', 'UTC')).toBe('2011-02-24T09:11:11');
        expect($filter('date')(rounded, 'yyyy-MM-ddTHH:mm:ss', 'UTC')).toBe('2011-02-24T09:00:00');
    });
    ...
    it('should truncate a time stamp to the exact hour; in the second half hour', function()
    {
        ...
        // THEN
        expect($filter('date')(timestamp, 'yyyy-MM-ddTHH:mm:ss', 'UTC')).toBe('2011-02-24T09:52:14');
        expect($filter('date')(rounded, 'yyyy-MM-ddTHH:mm:ss', 'UTC')).toBe('2011-02-24T09:00:00');
    });
